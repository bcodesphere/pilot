/**
 * Generated by orval 🍺
 * Do not edit manually.
 * Pilot API
 * API REST de Pilot, ERP multiempresa para PYMES de El Salvador. Incluye el núcleo (usuarios y empresas) y la app Contabilidad. Los montos viajan como cadena decimal con 2 decimales (ADR-013) y los errores siguen RFC 9457 (Problem Details).
 * OpenAPI spec version: 1.0.0
 */

/**
 * Monto en USD que puede ser negativo (p. ej. una diferencia), como cadena decimal con máximo 2 decimales (ADR-013). El servidor siempre responde con exactamente 2 decimales.
 * @pattern ^-?\d{1,17}(\.\d{1,2})?$
 */
export type MontoConSigno = string;
